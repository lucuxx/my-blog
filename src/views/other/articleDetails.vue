    ...
    <section v-if="show" class="articleDetails" v-loading="loading">
      ...
      <el-card class="card">
        <div slot="header" class="clearfix">
          ...
          <span class="desc"
            >最后编辑时间：{{
              $util.checkTime(currentArticleInfo.updateTime)
            }}</span
          >
          <span class="desc">分类：{{ currentArticleInfo.categoryName }}</span>
          <span class="desc"
            >作者：{{ currentArticleInfo.author || 'lxx' }}</span
          >
        </div>
        <div class="card-body">
...
https://skyclub.cloud/details?id={{ articleId }}</textarea
        ...
        <div class="next-body">
          <div
            class="item"
            @click="goNextArticle(_.get(articleData, `${currentIndex - 1}.id`))"
          >
            上一篇：{{
              articleData[currentIndex - 1]
                ? _.get(articleData, `${currentIndex - 1}.title`)
                : '暂无更多文章'
            }}
          </div>
          <div
      ...
      <el-card class="card">
        ...
        <div
          ...
          v-if="_.size(relevantArticle) > 0"
        >
          <div
            class="articleTab"
            style="width: 50%;height: 40px;line-height: 40px"
            v-for="(item, index) in relevantArticle"
            :key="item.id"
            @click="goNextArticle(item.id)"
          >
            {{ index + 1 }}、{{ item.title }}
          </div>
        </div>
...
export default class articleDetails extends Vue {
  ...
  getData(params: any) {
    this.currentArticleInfo = this.$lo.find(
      params,
      (item: any) => item.id === this.articleId
    )
  }
  ...
  async getArticleDetails() {
    ...
    this.isFabulous = isFabulous
      ? JSON.parse(isFabulous).indexOf(this.articleId) !== -1
      : false
    this.currentArticleInfo = await this.getArticle().then(
      (req: Types.InterfaceData) => this.$util.checkResp(req)
    )
    this.currentTagId = this.$lo.get(this.currentArticleInfo, 'tagId.0')
    const relevantArticle = await this.getTagArticle().then(
      (req: Types.InterfaceData) => this.$util.checkResp(req)
    )
    this.relevantArticle = this.$lo
      .filter(
        relevantArticle,
        (item: any) => item.id !== this.$lo.get(this.currentArticleInfo, 'id')
      )
      .slice(0, 4)
    this.$nextTick(() => {
    ...
    this.currentIndex = this.articleData.findIndex(
      (item: any) => item.id === this.$lo.get(this.currentArticleInfo, 'id')
    )
    setTimeout(() => {
      this.loading = false
  ...
  async clickFabulous() {
    if (this.isFabulous) {
      ...
      const { code } = await this.$api.updateArticle({
        id: this.articleId,
        fabulousNum: 1,
      })
      if (code === 0) {
        ...
        window.localStorage.setItem(
          'isFabulous',
          JSON.stringify(this.articleArr)
        )
